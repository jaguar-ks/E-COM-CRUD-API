# E2E Scenario 2: Inventory and Validation Rules

## Goal
Verify business rules for stock validation, duplicate protection, and delete constraints.

## Preconditions
- The API is running at `http://127.0.0.1:8000`.
- The database is seeded with the demo data.
- At least one category, customer, product, order, and order item already exist.

## Test Data
- Category name: `Automation Test Category`
- Customer email: `inventory.test@example.com`
- Product name: `Automation Test Product`
- Invalid stock quantity: a value larger than available inventory

## Steps
1. Create a new category with `POST /categories`.
2. Create a new customer with `POST /customers`.
3. Create a new product linked to the category with `POST /products`.
4. Create a valid order for the customer with `POST /orders`.
5. Create an order item that uses available product stock with `POST /order-items`.
6. Attempt to create another order item for the same product with a quantity that exceeds remaining stock.
7. Attempt to delete a product that is referenced by an order item.
8. Attempt to delete a customer that still has an order.

## Expected Results
- Category creation returns `201`.
- Customer creation returns `201`.
- Product creation returns `201`.
- Order creation returns `201`.
- The first order item creation returns `201` and reduces product stock.
- The oversized order item request is rejected with `409`.
- The referenced product delete request is rejected with `409`.
- The customer delete request is rejected with `409`.

## Postconditions
- Business rules remain enforced after the workflow runs.
- Stock levels and referential integrity stay consistent.
